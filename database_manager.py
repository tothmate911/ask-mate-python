...
@database_common.connection_handler
def search_in_questions_with_reputation(cursor, search_phrase):
    cursor.execute(F"""
        SELECT DISTINCT users.reputation, question.id, question.submission_time, question.view_number, question.vote_number, question.title, question.message, question.image, question.username
        FROM question 
    ...
    """)
    searched_question_with_reputation = cursor.fetchall()
    return searched_question_with_reputation


@database_common.connection_handler
def search_in_answers_with_reputation(cursor, search_phrase):
    cursor.execute(F"""
        SELECT * 
        FROM answer
        JOIN users ON answer.username = users.user_name
    ...
    """)
    searched_answer_with_reputation = cursor.fetchall()
    return searched_answer_with_reputation
